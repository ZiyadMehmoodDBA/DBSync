...
public sealed class NodeScopeService(AppDbContext db, IAuditService auditSvc) : INodeScopeService
{
    ...
    {
        // Fix 1: Guard — node must exist before any mutations
        if (!await db.Nodes.AnyAsync(n => n.NodeId == nodeId, ct))
            throw new NotFoundException($"Node '{nodeId}' not found.", "NODE_NOT_FOUND");

        var now = DateTime.UtcNow;

        // Fix 2: Wrap all mutations in an explicit transaction
        ...
        try
        {
            ...
            await auditSvc.WriteAsync(
                NodeManagementAuditActions.NodeScopeUpdated,
                $"node:{nodeId} channels:{req.ChannelIds.Length} triggers:{req.TriggerIds.Length} routers:{req.RouterIds.Length} dir:{req.SyncDirection} load:{req.InitialLoadPolicy}",
                actor, ct);
        }
        catch
        {
